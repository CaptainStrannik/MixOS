...
INT_PTR CALLBACK DialogProc(HWND h, UINT m, WPARAM w, LPARAM l)
{
	...
	switch (m) {
		...
		case WM_INITDIALOG:
			...
			// initialize the chosen output devices
			...
			}
			{
				...
				BASS_GetDeviceInfo(outdev[0], &info);
				MESS(20, WM_SETTEXT, 0, info.name);
				BASS_GetDeviceInfo(outdev[1], &info);
				MESS(21, WM_SETTEXT, 0, info.name);
			}
			return 1;
