...
static LRESULT CALLBACK wnd_proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        SetWindowText(hWnd, ("MusicPlayer Beta 1.75 © StrannikCorp"));
        CreateWindow(TEXT("STATIC"), TEXT("Желаете запустить?\n"), WS_VISIBLE | WS_CHILD, 550, 20, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
    ...
    case WM_COMMAND: {
        ...
        if (LOWORD(wParam) == ID_BUTTON)
        {
            ...
            CreateWindow(TEXT("STATIC"), TEXT(""), WS_VISIBLE | WS_CHILD, 550, 20, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            CreateWindow(TEXT("STATIC"), TEXT("                                    MusicPlayer Version Beta 1.75, copyright ©StrannikCorp. All rights reserved      "), WS_VISIBLE | WS_CHILD, 200, 20, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            CreateWindow(TEXT("STATIC"), TEXT("*******************************Привет, этот плеер проигрывает музыку в любых форматах ***********************************"), WS_VISIBLE | WS_CHILD, 200, 40, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            CreateWindow(TEXT("STATIC"), TEXT("                                                    Но расширение надо указывать обязательно"), WS_VISIBLE | WS_CHILD, 200, 60, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            CreateWindow(TEXT("STATIC"), TEXT("Тут уже имеется пара композиций"), WS_VISIBLE | WS_CHILD, 450, 80, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            ...
            CreateWindow(TEXT("BUTTON"), TEXT("Violet S - In your eyes "), WS_VISIBLE | WS_CHILD, 150, 230, 300, 70, hWnd, (HMENU)SLOVO, NULL, NULL);
            CreateWindow(TEXT("BUTTON"), TEXT("Violet S - Lonely "), WS_VISIBLE | WS_CHILD, 460, 230, 300, 70, hWnd, (HMENU)CIGARET, NULL, NULL);
            CreateWindow(TEXT("BUTTON"), TEXT("Violet S - Redhead Girl "), WS_VISIBLE | WS_CHILD, 770, 230, 300, 70, hWnd, (HMENU)PEREMEN, NULL, NULL);
        ...
        if (LOWORD(wParam) == DEVUSHKA)
        {
            BASS_Stop();
            BASS_Start();
            CreateWindow(TEXT("STATIC"), TEXT("Проигрывается Headless Nick - Morning"), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
        ...
        if (LOWORD(wParam) == LETO)
        {
            BASS_Stop();
            BASS_Start();
            CreateWindow(TEXT("STATIC"), TEXT("Проигрывается Headless Nick - UFO"), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
        ...
        if (LOWORD(wParam) == SHAG)
        {
            BASS_Stop();
            BASS_Start();
            CreateWindow(TEXT("STATIC"), TEXT("Проигрывается Headless Nick - Sample Adventures.mp3"), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
        ...
        if (LOWORD(wParam) == SLOVO)
        {
            BASS_Stop();
            BASS_Start();
            CreateWindow(TEXT("STATIC"), TEXT("Проигрывается Violet S - In your eyes.mp3"), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            EYES = BASS_StreamCreateFile(FALSE, "MusicPlayer\\Violet S\\In your Eyes.mp3", 0, 0, 0);
            BASS_ChannelStop(MORNING);
            BASS_ChannelStop(UFO);
            BASS_ChannelStop(SAMPLEADV);
            BASS_ChannelStop(EYES);
        ...
        if (LOWORD(wParam) == CIGARET)
        {
            BASS_Stop();
            BASS_Start();
            CreateWindow(TEXT("STATIC"), TEXT("Проигрывается Violet S - Lonely.mp3"), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
        ...
        if (LOWORD(wParam) == PEREMEN)
        {
            BASS_Stop();
            BASS_Start();
            CreateWindow(TEXT("STATIC"), TEXT("Проигрывается Violet S - Readhead Girl.mp3"), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            GIRL = BASS_StreamCreateFile(FALSE, "MusicPlayer\\Violet S\\Redhead Girl.mp3", 0, 0, 0);
            BASS_ChannelStop(MORNING);
            BASS_ChannelStop(UFO);
            BASS_ChannelStop(SAMPLEADV);
            BASS_ChannelStop(EYES);
        ...
        {
            CreateWindow(TEXT("STATIC"), TEXT("Музыка выключена"), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            BASS_ChannelStop(stro);
            BASS_ChannelStop(MORNING);
            BASS_ChannelStop(UFO);
            BASS_ChannelStop(SAMPLEADV);
        ...
        {
            MessageBoxA(hWnd, "Добро пожаловать в музыкальный плеер версии BETA 1.75. Который нужен, чтобы проигрывать музыку удобным образом. Хотя скорее для того, чтобы автор набрался опыта. Эта версия поддерживает большинство современных форматов музыки!! Каких именно? Да я сам хз =) Тестируйте, только не забывайте указывать расширение. =)", "О программе MusicPlayer Beta 1.75", MB_ICONINFORMATION);
        }
        ...
        {
            MessageBoxA(hWnd,"Не волнуйтесь, сейчас исходников нет. Автор занимается их форматированием. Будут доступны в момент релиза" ,"Исходный код",MB_OK );

        }
          if (LOWORD(wParam) == PAUSE1)
        {
            CreateWindow(TEXT("STATIC"), TEXT("Пауза                         "), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
          ...
          if (LOWORD(wParam) == PAUSE2)
        {
            ...
            CreateWindow(TEXT("STATIC"), TEXT("Проигрывается ваша музыка) "), WS_VISIBLE | WS_CHILD, 10, 600, 900, 25, hWnd, (HMENU)NULL, NULL, NULL);
            BASS_ChannelStop(MORNING);
            BASS_ChannelStop(UFO);
            BASS_ChannelStop(SAMPLEADV);
            BASS_ChannelStop(EYES);
